import { NODE_FAMILIES, NODE_META, NODE_ORDER } from "@/lib/meta";
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4">
      ...
      <p className="mt-0.5 text-[11px] text-slate-400">
        {NODE_ORDER.length} node types in {NODE_FAMILIES.length} families.
        Value, Principle, and Epistemic Limit are terminal — they end a chain.
        Premise is a root you reason forward from.
      </p>
      {NODE_FAMILIES.map((family) => (
        <div key={family.label} className="mt-3">
          <h3 className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
            {family.label} <span className="font-normal">— {family.hint}</span>
          </h3>
